...
public:
    ...
    std::map<LogLevel, int> countLevels(const std::vector<LogEntry>& entries) const;
    std::unordered_map<std::string, int> countMessageFrequency(const std::vector<LogEntry>& entries) const;
    std::vector<LogEntry> filterByLevel(const std::vector<LogEntry>& entries, LogLevel level) const;
